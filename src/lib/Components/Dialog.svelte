	...
	export const closeDialog = (): void => {
		const elements = Array.from(
			document.querySelectorAll(
				'button, input, select, textarea, a[href], [tabindex]:not([tabindex="0"])'
			)
	...
	<div class="dialog" transition:blur={{ duration: 200, easing: cubicInOut }} aria-disabled="false">
		<div class="dialogContent" class:mobileUi={!noMobileUi}>
			<div
				id="bg"
				class="dialogHeader"
				style="margin-top:10px;background-image:{image};background-color:var(--bg)"
			>
				<!-- todo: make this look like actual close button -->
				<button
					on:click={closeDialog}
					title={$t('LodestoneUI.Dialog.closeButton.title')}
					aria-label={$t('LodestoneUI.Dialog.closeButton.ariaLabel')}
					class="closeButton"><i class="fa-solid fa-x"></i></button
				>
